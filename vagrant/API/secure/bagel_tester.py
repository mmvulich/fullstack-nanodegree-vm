...
try:
	...
	url = address + '/users'
 	h = Http()
	#h.add_credentials('Madison', 'Udacity')
 	data = dict(username = "Madison", password = "Udacity")
...
print("Trying test two")
try:
    h = Http()
    h.add_credentials('Madison', 'Udacity')
    url = address + '/bagels'
    data = dict(username = "Madison", password = "Udacity", name = "plain", picture = "http://bonacbagel.weebly.com/uploads/4/0/5/4/40548977/s318635836612132814_p1_i1_w240.jpeg", description = "Old-Fashioned Plain Bagel", price = "$1.99")
...
try:
	h = Http()
	h.add_credentials('Madison','Youdacity')
	url = address + '/bagels'
	data = dict(username = "Small_Tim", password = "youdacity")
