...
db = psycopg2.connect(DBNAME)
c = db.cursor()
c.execute("select to_char(date, 'Month DD, YYYY'), trunc(percent, 2) as percent from (select t.date, (((e.error*1.0)/(t.views*1.0))*100.0)as percent from (select date(time) as date, count(*) as views from log group by date) as t, (select date(time) as date, count(status) as error from log where status = '404 NOT FOUND' group by date) as e where t.date = e.date) as final where percent > 1.0")
errors = c.fetchall()
